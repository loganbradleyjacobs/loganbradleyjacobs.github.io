...
This disassembly is not our user code, or is heavily obfuscated. Many of the comments left by radare2 are interpreting the values put into registers as characters, but they seem to be random, and don't correlate to anything we might expect to see, given how the program runs (No "P" for "Password" for example). Additionally, there are lots of indirect `jmp` commands, which may be written by a user intending to obfuscate their code, but also may be library code sorting out what to run.
